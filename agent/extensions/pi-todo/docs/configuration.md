...
The config is optional and is read only from `/Users/kazusa/.config/pi/agent/settings.json`,
under the top-level `"todo"` key.

A missing key, malformed settings JSON, JSON array, or non-object value is treated as `{}`,
so every default applies.

`todo` never writes this file — you create and edit it yourself, and the
...
`todo` reads no environment variables of its own.
