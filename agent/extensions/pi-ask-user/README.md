...
Local English-only Pi extension for structured user questionnaires.

Let the model ask you instead of guessing. This extension gives [Pi Agent](https://github.com/badlogic/pi-mono) one tool — `ask_user_question` — that opens a terminal dialog of up to four questions with written-out options, and hands your choices back as structured data.

## Install

This extension is installed locally in `/Users/kazusa/.config/pi/agent/extensions/pi-ask-user` and enabled through `/Users/kazusa/.config/pi/agent/settings.json`:

```json
{
  "extensions": ["extensions/pi-ask-user"]
}
```

Restart your Pi session after changing settings.
...
Optional. Settings live under the top-level `"ask-user-question"` key in `/Users/kazusa/.config/pi/agent/settings.json`; the file is read, never written.
...
```json
{ "ask-user-question": { "collapseKey": "alt+o" } }
```

Malformed settings fall back to the defaults; an individual unusable value is silently dropped back to its default. Never an error.
